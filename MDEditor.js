class MDEditor {
   ...
   #createToolbar() {
      if(this.contentId){
         ...
         this.toolbar.className = 'mdEditor-toolbar';
   ...
   getMDEditorContent() {
      ...
      const isInsideCodeBlock = el => {
         return el.closest('pre, code, .mdCode-block') !== null;
      };
      ...
      tempDiv.querySelectorAll('.mdCode-content').forEach(pre => {
   ...
   setMDEditorContent(html) {
      ...
      // Modify code blocks
      temp.querySelectorAll('.mdCode-content').forEach(pre => {
   ...
   if(this.contentId){
      ...
      this.editorArea.className = 'mdEditor-content';
   ...
   #addWatermark() {
      ...
      watermark.id = 'mdEditor-watermark';
      watermark.innerHTML = `Made with ❤️ by <a href="https://mramzanch.blogspot.com/" target="_blank" rel="noopener">M Ramzan Ch</a>`;
   ...
   #createDropdown(config) {
      ...
      select.className = `mdDropdown mdDropdown-${config.keyname}`;
   ...
   #createButton(config) {
      ...
      btn.className = `mdBtntn mdBtntn-${config.keyname}`;
         ...
         this.editorArea.addEventListener(evt, (e) => {
            ...
            if (!table) {
               ...
               document.querySelectorAll('table').forEach(t => {
                  t.classList.remove('mdEditor-table-highlighted');
               });
            }
         });
      });
   ...
   #showLinkPopup(anchor) {
      ...
      const existingPopup = document.querySelector('.mdEditor-link-edit-remove');
      if (existingPopup) existingPopup.remove();

      // Create popup element
      const popup = document.createElement('div');
      popup.className = 'mdEditor-link-edit-remove';
      popup.innerHTML = `
   ...
   #handleCodeBlockExit(e, elementNode) {
      ...
      const codeContent = elementNode.closest('.mdCode-content');
      ...
      if (text === '') {
         ...
         const codeBlock = codeContent.closest('.mdCode-block');
         if (codeBlock) {
   ...
   #handleYoutubeEmbed(e) {
      ...
      iframe.className = 'mdYoutube-embed';
   ...
   if (!this.editorArea.contains(selection.anchorNode)) return;
      ...
      wrapper.className = 'mdCode-block';

      // Create <pre><code> structure
      const pre = document.createElement('pre');
      pre.className = 'mdCode-content';
      pre.setAttribute('contenteditable', 'true');
      pre.setAttribute('spellcheck', 'false')
...
#openImageEditor(originalImg, wrapper) {
   ...
   editor.className = 'mdImg-editor-modal';
   ...
   #openLinkModal(anchor = null) {
      ...
      modal.className = 'mdEditor-link-modal';
      modal.innerHTML = `
   ...
   #openTableModal() {
      ...
      const modal = document.createElement('div');
      modal.className = 'mdEditor-table-modal';
      modal.innerHTML = `
         <div class="mdEditor-table-modal-content">
            <label>Rows: <input type="number" id="mdEditor-table-modal-rows" min="1" value="2" /></label>
            <label>Columns: <input type="number" id="mdEditor-table-modal-cols" min="1" value="2" /></label>
            <button id="mdEditor-insert-table">Insert Table</button>
            <button id="mdEditor-cancel-table">Cancel</button>
         </div>
      `;
      document.body.appendChild(modal);

      document.getElementById('mdEditor-insert-table').addEventListener('click', () => {
         const rows = parseInt(document.getElementById('mdEditor-table-modal-rows').value);
         const cols = parseInt(document.getElementById('mdEditor-table-modal-cols').value);
      ...
      document.getElementById('mdEditor-cancel-table').addEventListener('click', () => {
         modal.remove();
      });
   }
   ...
   #insertTable(rows, cols) {
      ...
      table.className = 'mdEditor-table';
      ...
      table.addEventListener('click', (e) => {
         ...
         document.querySelectorAll('table').forEach(t => t.classList.remove('mdEditor-table-highlighted'));

         // Add highlight to the selected table
         table.classList.add('mdEditor-table-highlighted');
      });
   ...
   #showTableContextMenu(td, x, y) {
      ...
      menu.className = 'mdTable-context-menu';
   ...
   #updateToolbar() {
      ...
      const checks = [
         { tag: 'b', style: ['font-weight', 'bold'], btn: 'mdBtntn-bold' },
         { tag: 'i', style: ['font-style', 'italic'], btn: 'mdBtntn-italic' },
         { tag: 'u', style: ['text-decoration', 'underline'], btn: 'mdBtntn-underline' },
         { tag: 'strike', style: ['text-decoration', 'line-through'], btn: 'mdBtntn-strikeThrough' },
         { tag: 'sub', style: null, btn: 'mdBtntn-subscript' },
         { tag: 'sup', style: null, btn: 'mdBtntn-superscript' },
         { tag: 'li', style: null, btn: 'mdBtntn-orderedList' },
         { tag: 'ol', style: null, btn: 'mdBtntn-unorderedList' },
         { tag: 'font', style: ['color'], btn: 'mdBtntn-textColor' },
         { tag: 'code', style: null, btn: 'mdBtntn-codeInline' }
      ];
      ...
      if (parent.closest('.mdCode-block')) {
         document.getElementById('mdBtntn-codeBlock')?.classList.add('active');
      }
      ...
      const headingSelect = document.querySelector('.mdDropdown-heading');
      ...
      const fontSelect = document.querySelector('.mdDropdown-fonts');
      if (fontSelect) {
      ...
      const alignmentSelect = document.querySelector('.mdDropdown-textAlignment');
   ...
   #resetToolbar() {
      document.querySelectorAll('.mdEditor-toolbar button').forEach(btn => {
   ...
   #handleMention(username) {
      ...
      mentionNode.className = 'mention mdMention';
      ...
      content = content.replace(regexReplace, (match, username) => {
         ...
         mention.className = 'mention mdMention';
   ...
   #includeCSS(){
      ...
      let styleCount = document.querySelector('.mdEditor-styles')
      if(!styleCount || styleCount.length === 0){
      const style = document.createElement('link');
      style.rel = 'stylesheet';
      style.type = 'text/css';
      style.className = 'mdEditor-styles';
      style.href = 'https://cdn.jsdelivr.net/gh/yeole-rohan/mdEditor@main/mdEditor.css';
    ...
    #setToolbarType(){
      ...
      this.toolbar.classList.add(`mdEditor-toolbar-${this.options.toolbarType}`);
      this.toolbar.id = 'mdEditor-toolbar-'+this.toolbarIndex;
      this.toolbarIndex++;
   ...
   const oldOverflowBtn = this.toolbar.querySelector('.mdBtntn-overflowMenu');
   const oldDropdown = this.toolbar.querySelector('.mdToolbar-overflow-dropdown');
   ...
   let buttons = Array.from(this.toolbar.querySelectorAll('button:not(.mdBtntn-overflowMenu)'));
   ...
   overflowBtn.className = 'mdBtntn mdBtntn-overflowMenu';
   ...
   dropdown.className = 'mdToolbar-overflow-dropdown';
   ...
   if (btn.tagName === 'SELECT') {
      ...
      const keyname = Array.from(btn.classList).find(cls => cls.startsWith('mdDropdown-'));
      let configKey = keyname ? keyname.replace('mdDropdown-', '') : null;
   ...
   const keyname = btn.id.replace('mdBtntn-', '');
...
#toggleSourceMode() {
   ...
   if (!this.isSourceMode) {
      ...
      this.sourceTextarea.className = 'mdEditor-sourcearea';
